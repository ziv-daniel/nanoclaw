...
async function sendPairingConfirmation(token: string, platformId: string): Promise<void> {
  ...
  try {
    ...
    log.warn('Pairing confirmation failed', { err });
  }
}
...
function createPairingInterceptor(
  ...
  return async (platformId, threadId, message) => {
    try {
      ...
      log.info('Telegram pairing accepted', {
        platformId,
...
function createEmojiAckInterceptor(token: string, nextOnInbound: ChannelSetup['onInbound']): ChannelSetup['onInbound'] {
  return (platformId, threadId, message) => {
    try {
...
async function sendDoneReaction(token: string, chatId: string, messageId: number): Promise<void> {
  try {
    ...
    log.debug('done reaction failed', { err });
  }
}
    ...
    const wrapped: ChannelAdapter = {
      ...
      async setup(hostConfig: ChannelSetup) {
        const intercepted: ChannelSetup = {
          ...
          onInbound: createEmojiAckInterceptor(
            token,
            createForumTopicInterceptor(createPairingInterceptor(botUsernamePromise, hostConfig.onInbound, token)),
          ),
        };
      ...
      async deliver(
        ...
        try {
          ...
          log.debug('done reaction wiring failed', { err });
        }
        return result;
      },
    };
